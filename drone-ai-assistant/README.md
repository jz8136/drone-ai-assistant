...
不只是套壳对话，而是垂直领域的 AI 落地实践。
本项目是一个专为无人机维保场景打造的 Web AI 系统。通过全链路架构，实现了从海量二进制飞控日志上传 -> Node.js 核心数据提取 -> 动态 Prompt 组装 -> AI 专家分析 -> 前端 SSE 实时流式渲染的完整闭环。

✨ 核心亮点 (Core Highlights)
本项目拒绝简单的 API 拼接，深入解决了复杂 AI 前端交互中的多项工程化痛点：

⚡ 海量对话长列表优化：采用 react-window 实现虚拟列表 (Virtual List)，使包含上千条记录的历史会话侧边栏维持毫秒级切换，真实 DOM 节点始终控制在个位数，彻底杜绝页面卡顿。
...
🛡️ 高可用中间层设计：Node.js 后端不仅负责文件 multer 接收与降维解析，还实现了指数退避 (Exponential Backoff) + Jitter 随机抖动算法的防弱网重试机制，有效保护第三方 AI 接口配额。
...
4. 生产环境构建体验
   ...
   npm run preview
   🧪 核心使用场景展示
   日常排障问答：在底部的输入框内输入故障现象（如：“电机起飞时发出异响，且伴随 GPS 丢失”），体验极速的流式打字机回复。

日志深度解析：点击输入框左侧的 📎 按钮，上传一份假定的 .ulog 文件，等待系统自动提取数据并输出极具专业性的排障指引。

会话管理：随意新建、切换、删除侧边栏的历史记录，体验在极端复杂状态下的丝滑 UI。
